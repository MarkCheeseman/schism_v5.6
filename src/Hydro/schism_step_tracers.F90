...
!$OMP   do
        do i=1,nea
          ...
          !settling vel in internal prisms (positive downward) #REMY#
          do k=itmp1,itmp2
            wsett(k,:,i)=gen_wsett(k-itmp1+1) !*sin(2*pi*time/10/86400)
          enddo
...
!       principle); at Surface layer #REMY#
...
!$OMP     do
          do i=1,nea
            ...
            bigv=area(i)*(ze(nvrt,i)-ze(nvrt-1,i))
            if(bigv<=0) call parallel_abort('STEP: bigv==0 (3)')
            rat=vsource(i)*dt/bigv !ratio of volumes (>0)
            do j=1,ntracers
              if(msource(j,i)>-99) tr_el(j,nvrt,i)=(tr_el(j,nvrt,i)+rat*msource(j,i))/(1+rat)
            enddo !j
          enddo !i
!$OMP     end do
        endif !if_source
