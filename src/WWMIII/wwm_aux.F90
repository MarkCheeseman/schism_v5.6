...
!             CALL ALL_FROM_TABLE(FP,DEP(IP),KPP,CGP,KD,WNP,CP)
          ...
          ELSE 
            ...
            IF (CONVK1 .LT. EPSH1) IPCONV(1) = IPCONV(1) + 1 
            IF (CONVK2 .LT. EPSH2) IPCONV(2) = IPCONV(2) + 1
...
#ifdef MPI_PARALL_GRID
        call mpi_allreduce( MPI_IN_PLACE, IPCONV, 5, itype, MPI_SUM, COMM, ierr )
        conv_array = MyREAL(IPCONV) / MyREAL(NP_GLOBAL) 
        conv_array = conv_array * 100.0
