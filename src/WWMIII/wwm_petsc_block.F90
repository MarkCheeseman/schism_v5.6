...
!> Data and methods to use one matrix for all nodes, frequency and directions.
...
! create ghost blocks
        ...
        if (istat /= 0) then 
          CALL WWM_ABORT('allocation error in wwm_petsc_block 1')
        endif
        nghostBlock = 0
        ...
        do ISS = 1, NUMSIG
        do IDD = 1, NUMDIR
           onlyGhostsBlock(toRowIndex(IP, ISS, IDD)) = &
        &                         toRowIndex(onlyGhosts(IP-1), ISS, IDD)
        end do
        end do
        end do
